...
const ModuleFactory GattHal::Factory = ModuleFactory([]() { return new GattHalImpl(); });

struct GattHalImpl::impl {
  ~impl() {}
  void start() {
  ...
  bool is_bound() const { return gatt_hal_instance_ != nullptr; }
  ...
  bool initialize(hal::GattHalCallback const* callback) {
    if (!is_bound()) {
      return false;
    }
  ...
  bool register_service(const hal::GattSession& session) const {
    if (!is_bound()) {
      return false;
    }
    log::info(
  ...
  void unregister_service(int session_id) const {
    if (!is_bound()) {
      return;
    }
  ...
  void clear_services(int acl_connection_handle) const {
    if (!is_bound()) {
      return;
    }
...
GattHalImpl::GattHalImpl() : pimpl_(std::make_unique<impl>()) {}

GattHalImpl::~GattHalImpl() = default;

void GattHalImpl::ListDependencies(ModuleList* list) const {
  (void)list;
}

void GattHalImpl::Start() {
  log::info("Starting GattHalImpl");
  pimpl_->start();
}

void GattHalImpl::Stop() {
  log::info("Stopping GattHalImpl");
  pimpl_->stop();
}

std::string GattHalImpl::ToString() const {
  return "Gatt HAL";
}

bool GattHalImpl::Initialize(hal::GattHalCallback const* callback) {
  return pimpl_->initialize(callback);
}
...
bool GattHalImpl::RegisterService(const hal::GattSession& session) const {
  return pimpl_->register_service(session);
}

void GattHalImpl::UnregisterService(int session_id) const {
    pimpl_->unregister_service(session_id);
}

void GattHalImpl::ClearServices(int acl_connection_handle) const {
  pimpl_->clear_services(acl_connection_handle);
}
