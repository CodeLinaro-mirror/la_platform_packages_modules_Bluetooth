...
static const char* interop_feature_string_(const interop_feature_t feature) {
  switch (feature) {
    ...
    CASE_RETURN_STR(INTEROP_IGNORE_SERVICE_CHANGED_IND);
    ...
    CASE_RETURN_STR(INTEROP_AVDTP_SKIP_DISCOVER_AFTER_CONFIG);
    ...
    CASE_RETURN_STR(INTEROP_DELAY_ROLE_SWITCH_STILL_ENCRYPTION);
    CASE_RETURN_STR(INTEROP_HOGP_RECONNECT_ON_FIRST_CONNECTION);
  }
...
void interop_database_add_addr(const uint16_t feature, RawAddress addr, size_t length) {
  ...
  memcpy(&entry->entry_type.addr_entry.addr, addr.address.data(), length);
  entry->entry_type.addr_entry.feature = (interop_feature_t)feature;
