...
public:
  ...
  using StartSyncCb = base::Callback<void(uint8_t status, uint16_t sync_handle,
                                          uint8_t advertising_sid, uint8_t address_type,
                                          RawAddress address, uint8_t phy, uint16_t interval)>;
  using SyncReportCb = base::Callback<void(uint16_t sync_handle, int8_t tx_power, int8_t rssi,
  ...
  using SyncTransferCb = base::Callback<void(uint8_t /*status*/, RawAddress /*addr*/)>;
