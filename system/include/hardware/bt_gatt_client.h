...
typedef struct {
  ...
  /** Create a BLE V2 connection to a remote LE*/
  bt_status_t (*connect_v2)(int client_if, const RawAddress& bd_addr, uint8_t addr_type,
                         bool is_direct, int transport, bool opportunistic, int initiating_phys,
                         int preferred_mtu, uint8_t pa_handle, uint8_t sub_event, uint8_t filter_policy);
#endif
