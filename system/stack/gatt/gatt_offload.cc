...
static bool contains_session_by_acl_handle(uint16_t acl_connection_handle);
static bool contains_session_by_id(uint16_t session_id);
static bool contains_session_by_id_conn_id_to_unoffload(uint16_t session_id, tCONN_ID conn_id);
...
void gatt_unoffload_session(tCONN_ID conn_id, uint16_t session_id, tGATT_STATUS status) {
  ...
  if (!contains_session_by_id_conn_id_to_unoffload(session_id, conn_id)) {
    log::info("session_id: {}, conn_id: {} doesn't exist or unoffloading is already in progress",
              session_id, conn_id);
    return;
  }
  gatt_cb.offload_sessions[session_id].in_unoffloading = true;
...
bool gatt_offload_clear_sessions_by_acl_handle(uint16_t acl_connection_handle) {
  log::info("acl_connection_handle: 0x{:x}", acl_connection_handle);
  if (!contains_session_by_acl_handle(acl_connection_handle)) {
    log::warn("acl_connection_handle: 0x{:x} doesn't exist", acl_connection_handle);
    return false;
  }
...
static bool contains_session_by_id_conn_id_to_unoffload(uint16_t session_id, tCONN_ID conn_id) {
  ...
  return iter != gatt_cb.offload_sessions.end() && !iter->second.in_unoffloading;
}
