...
typedef enum : uint8_t {
  ...
  HCI_ERR_INSUFFICIENT_SECURITY = 0x2F,       // btif/btu
...
inline std::string hci_error_code_text(const tHCI_ERROR_CODE& error_code) {
  switch (error_code) {
    ...
    CASE_RETURN_TEXT(HCI_ERR_ROLE_CHANGE_NOT_ALLOWED);
    ...
    CASE_RETURN_TEXT(HCI_ERR_INSUFFICIENT_SECURITY);
...
inline bt_status_t hci_error_to_bt_status(const tHCI_ERROR_CODE& error_code) {
  switch (error_code) {
    ...
    case HCI_ERR_UNSPECIFIED: /* 0x1F */
      return BT_STATUS_FAIL;
    case HCI_ERR_ROLE_CHANGE_NOT_ALLOWED: /* 0x21 */
      return BT_STATUS_UNSUPPORTED;
