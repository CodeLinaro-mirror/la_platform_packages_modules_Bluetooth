...
public:
  ...
  bool is_encryption_switching() const { return encrypt_state_ == BtmAclEncryptState::kTemporaryOff;}
...
public:
  ...
  unsigned NumberOfActiveBredrLinks() const {
    unsigned cnt = 0;
    for (int i = 0; i < MAX_L2CAP_LINKS; i++) {
      if ((acl_db[i].InUse()) && (acl_db[i].link_spec.transport == BT_TRANSPORT_BR_EDR)) {
        ++cnt;
      }
    }
    return cnt;
