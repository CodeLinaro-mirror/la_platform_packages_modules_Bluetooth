...
public class HeadsetService extends ConnectableProfile {
    ...
    private BluetoothDevice mPreviousActiveDevice = null;
    ...
    private void removeActiveDevice() {
        synchronized (mStateMachines) {
            ...
            mPreviousActiveDevice = mActiveDevice;
            mActiveDevice = null;
    ...
    public boolean setActiveDevice(
        ...
        synchronized (mStateMachines) {
            ...
            if (getAudioState(mPreviousActiveDevice) != BluetoothHeadset.STATE_AUDIO_DISCONNECTED) {
                int disconnectStatus = disconnectAudio(mPreviousActiveDevice);
                if (disconnectStatus != BluetoothStatusCodes.SUCCESS) {
                    Log.e(
                            TAG,
                            "setActiveDevice: fail to disconnectAudio from "
                                    ...
                                    + " with status code "
                                    + disconnectStatus);
                    mActiveDevice = mPreviousActiveDevice;
                    mNativeInterface.setActiveDevice(mPreviousActiveDevice);
                    return false;
                }
        ...
        synchronized (mStateMachines) {
            ...
            if (!leAudioService.isEmpty()
                ...
                isActiveLeAudioDeviceFound =
                   leAudioService.get().IsActiveLeAudioDeviceExistCacheVrHfpDevice(fromDevice);
            }
    ...
    public void onConnectionStateChangedFromStateMachine(
        ...
        if (mBluetoothAgClientService != null) {
           Log.d(TAG," onConnectionStateChangedFromStateMachine: AgClientService is not null ");
    ...
    public void updateInbandRinging(BluetoothDevice device, boolean connected) {
        synchronized (mStateMachines) {
            ...
            if (isHeadsetClientConnected() || mActiveDevice == null) {
            ...
            if (getConnectedDevices().size() > 1 && !mInbandRingingRuntimeDisable) {
                if (mActiveDevice != null) {
                    doForStateMachine(mActiveDevice,
                        sm -> sm.sendMessage(HeadsetStateMachine.SEND_BSIR, 1));
                }
                if (mPreviousActiveDevice != null && !mPreviousActiveDevice.equals(mActiveDevice)) {
                    doForStateMachine(mPreviousActiveDevice,
                        sm -> sm.sendMessage(HeadsetStateMachine.SEND_BSIR, 0));
                }
            } else {
        ...
        synchronized (mStateMachines) {
            if (toState == BluetoothHeadset.STATE_AUDIO_DISCONNECTED) {
                ...
                wrapper.isCallIdleAndScoNotManagedbyHal =
                       (mSystemInterface.isCallIdle() && !mSystemInterface.isScoManagedByAudioEnabled());
            }
            BluetoothAgClientService mBluetoothAgClientService = 
                           BluetoothAgClientService.getBluetoothAgClientService();
            if (mBluetoothAgClientService != null) {
