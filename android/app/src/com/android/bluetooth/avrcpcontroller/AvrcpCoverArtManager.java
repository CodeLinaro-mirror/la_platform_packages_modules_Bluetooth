...
class AvrcpCoverArtManager {
    ...
    private int mHeight, mWidth, mMimeType;
    ...
    public synchronized void updateImageProperties() {
        mType = SystemProperties.get(
            AVRCP_CONTROLLER_COVER_ART_IMGTYPE, IMAGE_TYPE_DEFAULT);
        mDownloadScheme = SystemProperties.get(
            AVRCP_CONTROLLER_COVER_ART_SCHEME, SCHEME_DEFUALT);
        mMimeType = BipEncoding.getEncodingTypeFromString(
            SystemProperties.get(AVRCP_CONTROLLER_COVER_ART_MIMETYPE, MIMETYPE_DEFAULT));
        mHeight = SystemProperties.getInt(
            AVRCP_CONTROLLER_COVER_ART_IMGHEIGHT, IMAGE_HEIGHT_DEFAULT);
        mWidth = SystemProperties.getInt(
            AVRCP_CONTROLLER_COVER_ART_IMGWIDTH, IMAGE_WIDTH_DEFAULT);
    }
